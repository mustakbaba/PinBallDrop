...
public class SettingsView extends BaseDialog<SettingsDialogModel> {
    ...
    public void configureWithModel(SettingsDialogModel model) {
        ...
        if (model.showCMPButton) {
            addUnitySendMessageButton();
        }
        
		idStr = model.elephantId;
		addCopyIdButton();
    }
    ...
    private void addUnitySendMessageButton() {
        RollicButton unityButton = new RollicButton(getContext(), "Consent Settings");
        unityButton.setMinHeight(Utils.dpToPx(60));
        unityButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                com.unity3d.player.UnityPlayer.UnitySendMessage("Elephant", "ShowSecondLayer", "Pressed");
            }
        });

        complianceActionButtonsLayout.addView(unityButton, buttonLayoutParams);
    }

    private void addCopyIdButton() {
        RollicButton copyButton = new RollicButton(getContext(), "Copy ID\n" + idStr);
        copyButton.setMinHeight(Utils.dpToPx(60));
        copyButton.setTextSize(14.0f);
        copyButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ClipboardManager clipboard = (ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE);
                ClipData clip = ClipData.newPlainText("Copied ID", idStr);
                clipboard.setPrimaryClip(clip);
            }
        });

        complianceActionButtonsLayout.addView(copyButton, buttonLayoutParams);
    }
